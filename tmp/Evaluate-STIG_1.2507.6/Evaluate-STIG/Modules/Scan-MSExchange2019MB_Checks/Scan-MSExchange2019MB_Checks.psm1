...
Function Get-V259645 {
    ...
    ForEach ($Item in $RpcClientAccess) {
        If ($Item.$($Setting) -eq $ExpectedValue) {
            $FindingDetails += "$($Item.Server.Name)" | Out-String
...
Function Get-V259646 {
    ...
    $Access = Powershell.exe -Command $CommandSB

    $FindingDetails += "IIS Service Certificates" | Out-String
    $FindingDetails += "---------------------------" | Out-String
    If (($ExchangeCertificate).ToString().Length -gt 0) {
        # Certs must be verified manually if trusted public
        ForEach ($Cert in $ExchangeCertificate) {
            $FindingDetails += "Subject:`t`t$($Cert.Subject)" | Out-String
            $FindingDetails += "Services:`t`t$($Cert.Services.ToString())" | Out-String
    ...
    $FindingDetails += "---------------------------" | Out-String
    ForEach ($Item in $OwaVirtualDirectory) {
...
Function Get-V259648 {
    ...
    $FindingDetails += "$($AdminAuditLogConfig.Name)" | Out-String
    If ($AdminAuditLogConfig.$($Setting) -eq $ExpectedValue) {
        $Status = "NotAFinding"
        $FindingDetails += "$($Setting):`t$($AdminAuditLogConfig.$($Setting))" | Out-String
    }
    Else {
        $Status = "Open"
        $FindingDetails += "$($Setting):`t$($AdminAuditLogConfig.$($Setting)) [Expected $($ExpectedValue)]" | Out-String
    }
    #---=== End Custom Code ===---#
...
Function Get-V259651 {
    ...
    Else {
        ...
        ForEach ($Item in $RemoteDomain) {
            If ($Item.$($Setting) -eq $ExpectedValue -or $Item.DomainName.ToString() -like "*.mil" -or $Item.DomainName.ToString() -like "*.gov") {
                $FindingDetails += "$($Item.Name)" | Out-String
                $FindingDetails += "DomainName:`t`t`t$($Item.DomainName.ToString())" | Out-String
                $FindingDetails += "$($Setting):`t$($Item.$($Setting))" | Out-String
                $FindingDetails += "" | Out-String
            }
            Else {
...
Function Get-V259654 {
    ...
    $FindingDetails += "$($AdminAuditLogConfig.Name)" | Out-String
    If ([String]$AdminAuditLogConfig.$($Setting) -eq $ExpectedValue) {
        $Status = "NotAFinding"
        $FindingDetails += "$($Setting):`t$($AdminAuditLogConfig.$($Setting))" | Out-String
    }
    Else {
...
Function Get-V259655 {
    ...
    If (($RecordsManagementGroup).ToString().Length -gt 0) {
        $FindingDetails += "Members of role group '$RoleGroup':" | Out-String
        $FindingDetails += "---------------------------" | Out-String
        ForEach ($Item in $RecordsManagementGroup) {
            $FindingDetails += "Name:`t`t$($Item.Name)" | Out-String
            $FindingDetails += "RecipientType:`t$($Item.RecipientType.ToString())" | Out-String
            $FindingDetails += "" | Out-String
        }
    }
    Else {
...
Function Get-V259658 {
    ...
    ForEach ($Item in $MailboxDatabase) {
        If ($Item.$($Setting) -eq $ExpectedValue) {
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "$($Setting):`t$($Item.$($Setting))" | Out-String
            $FindingDetails += "" | Out-String
...
Function Get-V259672 {
    ...
    If (($ForwardingSmtpAddress).ToString().Length -gt 0) {
        ForEach ($Item in $ForwardingSmtpAddress) {
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "$($Setting):`t$($Item.$($Setting))" | Out-String
            $FindingDetails += "" | Out-String
        }
...
Function Get-V259682 {
    ...
    If ($TransportConfig.$($Setting).$($Prop1) -eq $ExpectedValue1 -and $TransportConfig.$($Setting).$($Prop2) -eq $ExpectedValue2) {
        $FindingDetails += "$($Setting)" | Out-String
        $FindingDetails += "$($Prop1):`t$($TransportConfig.$($Setting).$($Prop1))" | Out-String
        $FindingDetails += "$($Prop2):`t`t$($TransportConfig.$($Setting).$($Prop2))" | Out-String
    }
    Else {
        $Compliant = $false
        $FindingDetails += "$($Setting)" | Out-String
        If ($TransportConfig.$($Setting).$($Prop1) -eq $ExpectedValue1) {
            $FindingDetails += "$($Prop1):`t$($TransportConfig.$($Setting).$($Prop1))" | Out-String
        }
        Else {
            $FindingDetails += "$($Prop1):`t$($TransportConfig.$($Setting).$($Prop1)) [Expected $($ExpectedValue1)]" | Out-String
        }
        If ($TransportConfig.$($Setting).$($Prop2) -eq $ExpectedValue2) {
            $FindingDetails += "$($Prop2):`t`t$($TransportConfig.$($Setting).$($Prop2))" | Out-String
        }
        Else {
            $FindingDetails += "$($Prop2):`t`t$($TransportConfig.$($Setting).$($Prop2)) [Expected $($ExpectedValue2)]" | Out-String
        }
    }
...
Function Get-V259688 {
    ...
    ForEach ($Item in $RemoteDomain) {
        if ($Item.$($Setting).ToString() -eq $ExpectedValue) {
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "DomainName:`t`t$($Item.DomainName.ToString())" | Out-String
            $FindingDetails += "$($Setting):`t$($Item.$($Setting).ToString())" | Out-String
            $FindingDetails += "" | Out-String
        }
        Else {
...
Function Get-V259691 {
    ...
    If ($TransportConfig.$($Setting)) {
        $FindingDetails += "$($Setting):" | Out-String
        ForEach ($Item in $TransportConfig) {
...
Function Get-V259694 {
    ...
    .DESCRIPTION
        ...
        CCI ID     : CCI-001247, CCI-002624, CCI-004964
        Rule Name  : SRG-APP-000272
        Rule Title : Exchange antimalware agent must be enabled and configured.
        DiscussMD5 : B9E95A657EB7022302F6712B770885A1
    ...
    If (($MalwareAgent).ToString().Length -gt 0) {
        If ($MalwareAgent.$($Prop) -eq $ExpectedValue) {
            $Status = "NotAFinding"
            $FindingDetails += "$($Setting)" | Out-String
            $FindingDetails += "$($Prop):`t$($MalwareAgent.$($Prop))" | Out-String
        }
        Else {
            $Status = "Open"
            $FindingDetails += "$($Setting)" | Out-String
            $FindingDetails += "$($Prop):`t$($MalwareAgent.$($Prop)) [Expected $($ExpectedValue)]" | Out-String
        }
    }
    Else {
        $Status = "NotAFinding"
...
Function Get-V259695 {
    ...
    If (($MalwareFilteringServer).ToString().Length -gt 0) {
        ForEach ($Item in $MalwareFilteringServer) {
...
Function Get-V259703 {
    ...
    If (($OutlookAnywhere).ToString().Length -gt 0) {
        ForEach ($Item in $OutlookAnywhere) {
            $FindingDetails += "$($Item.Identity)" | Out-String
            ForEach ($Setting in $Settings) {
                If ($Item.$($Setting).ToString() -eq $ExpectedValue) {
                    $FindingDetails += "$($Setting):`t$($Item.$($Setting).ToString())" | Out-String
...
Function Get-V259706 {
    ...
    ForEach ($Item in $RemoteDomain) {
        If ($Item.$($Setting) -eq $ExpectedValue) {
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "DomainName:`t$($Item.DomainName.ToString())" | Out-String
            $FindingDetails += "$($Setting):`t$($Item.$($Setting))" | Out-String
            $FindingDetails += "" | Out-String
        }
        Else {
...
Function Get-V259709 {
    ...
    #---=== Begin Custom Code ===---#
    If (($DatabaseAvailabilityGroup).ToString().Length -gt 0) {
        ForEach ($Item in $DatabaseAvailabilityGroup) {
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "Servers:`t`t$($Item.Servers -join ', ')" | Out-String
            $FindingDetails += "WitnessServer:`t$($Item.WitnessServer.ToString())" | Out-String
            $FindingDetails += "" | Out-String
        }
    }
    Else {
...
Function Get-V259710 {
    ...
    ForEach ($Item in $ReceiveConnector) {
        If ($ExpectedValue -in ($Item.$($Setting).ToString() -split (",\s*"))) {
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "$($Setting):`t$($Item.$($Setting).ToString())" | Out-String
            $FindingDetails += "" | Out-String
        }
        Else {
            $Compliant = $false
            $FindingDetails += "$($Item.Name)" | Out-String
            $FindingDetails += "$($Setting):`t$($Item.$($Setting).ToString()) [Missing $($ExpectedValue)]" | Out-String
            $FindingDetails += "" | Out-String
        }
    }
...
Function Get-V259711 {
    ...
    ForEach ($Item in $ExchangeServer) {
        $FindingDetails += "$($Item.Name)" | Out-String
        $FindingDetails += "AdminDisplayVersion:`t$($Item.$($Setting).ToString())" | Out-String
        $FindingDetails += "" | Out-String
    }
    #---=== End Custom Code ===---#
...
Function Get-V259712 {
    ...
    $WebSite = Get-Website | Where-Object Name -ne "Exchange Back End"

    $Sites = [System.Collections.Generic.List[System.Object]]::new()
    ForEach ($SiteObject in $WebSite) {
        $Bindings = [System.Collections.Generic.List[System.Object]]::new()
        ForEach ($Item in $SiteObject.bindings.Collection) {
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCDwkg74DuNmyWT5
# ehCe6CrluOAYJbTzEn5NuwfFcna7rKCCHiQwggUqMIIEEqADAgECAgMTYdUwDQYJ
...
# BCCTz4J9DKTp5cyc2WMyqGtmbp1z4Eh9ztSoJk9pDVSoRjANBgkqhkiG9w0BAQEF
# AASCAQB5hce6yR78Jm61wBN+OdFMQ181aluE/QUfTy+Q4T+cgh6UhPS+CGwfGi+g
# WysT1zMdJotrBTZaTVFIjO9Lhm2Aq6DJcy6YHYpXCdwp8Yc7mx4YWivU5wwUau7X
# JWDhy/8ub7AWT8Nj5NMxe25JH97wUsK2vf0WZCKGDs3oj4MIb+ywjK+OaJgRWUQc
# FbwLbadNUiYYNkl2yqX5llNaTyQFjkKPNpEwV/StG2JPCe1MMUL2eh6za8fbWU7y
# EmGPj5Bvpga4nbcA3IldXQ5S8N9iA7hVFCE9JsbA0+51CYrafrAFlwr9Zmk+XNUw
# duYkr/00qfvUR79Ddv7oE77F19ZvoYIDJjCCAyIGCSqGSIb3DQEJBjGCAxMwggMP
# AgEBMH0waTELMAkGA1UEBhMCVVMxFzAVBgNVBAoTDkRpZ2lDZXJ0LCBJbmMuMUEw
# PwYDVQQDEzhEaWdpQ2VydCBUcnVzdGVkIEc0IFRpbWVTdGFtcGluZyBSU0E0MDk2
# IFNIQTI1NiAyMDI1IENBMQIQCoDvGEuN8QWC0cR2p5V0aDANBglghkgBZQMEAgEF
# AKBpMBgGCSqGSIb3DQEJAzELBgkqhkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTI1
# MDkxNTEwNTYxOVowLwYJKoZIhvcNAQkEMSIEILikWsem/ZlUJITrMH4iQqAZaM1d
# XKaYNuLmGPqwNEETMA0GCSqGSIb3DQEBAQUABIICAMYLF6dEx2GYHwISSpvwe/pz
# zuV3zSuQ7uALjEoIPlPPogyAuKcMnXujhRHHoDR1vOfjVVLdp9oayKAeddIRepSQ
# at4hbwmMYQ3oUZUfVsIQbFeTxKgPKsJZob1CVNopIXPIBADAZMFkGzWGoKfc+Voi
# fCbekwkJ2rHjjB/Vt+pIfTs5pWe7A2LfgNYdrcnKoF5xV9faRtFSdQeVmx4QGgkM
# yWfKAqvJGXGV5COyua7eDKS3xwVO9Hlh7NV8G6dVxY6deTM0o+hgIlnppbnH+oMR
# fwady9SPgPFFt9kpC9KLSACrz+leAKNIahRyVBOStWsEKReqfqam5CRsJXicRIju
# GtlJ6eONres2gJTQDJ1g99bpfG0npnFdw7JCtJ/NH8Y/J687dwM3tG+nIdh06sAD
# 2WVRunMrN1y0m6eGZ5Dtia3zTlwhn2dfPcWHziBc+PPdpN0TavhqWnb+CGRrBrXm
# idQEXe8biH0dJh8SLkiCSh36B5DKsZbtt9bpjGnH3Cs6/nk7Gcha2nn4sDP2+IXg
# 0ZWAJ+siQdFOUfktDqYvWe5Ob8YFvi1qdvTf83wTbzn6Z8vFgdSW45cj5qQTfvDQ
# z2BVzf7irtlANn/w32eYD4AYwV+QmDSPDc3Qk8Yi+psXIi+QaoG+Ii3q7lOCKBkV
# CPiOigaPnoNpEnRugeIT
